...
public class UserService {
    ...
    public DynamicArray findAllUserAndOrderBy(OrderType orderType) throws SQLException {
        String orderDirection;
        if (orderType.equals(OrderType.ASC))
            orderDirection = OrderType.ASC.name();
        else if (orderType.equals(OrderType.DESC))
            orderDirection = OrderType.DESC.name();
        else
            return null;
        return userRepository.findAllByOrder(orderDirection);
    }
}
